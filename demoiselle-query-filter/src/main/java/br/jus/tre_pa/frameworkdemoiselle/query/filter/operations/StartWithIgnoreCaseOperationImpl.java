...
public class StartWithIgnoreCaseOperationImpl implements SingularOperation<String> {

	@Override
	public <T> Predicate execute(CriteriaBuilder cb, Root<T> p, String field, String value) {
		return !Strings.isEmpty(value) ? cb.like(cb.lower(p.<String> get(field)), value.concat("%").toLowerCase()) : null;
	}
}
